...
class dwx_client():
            ...
            for order_id, order in data['orders'].items():
                if order_id not in self.open_orders:
                    ...
                    if self.verbose:
                        ...
                        self.open_order(symbol=order['symbol'], order_id=order_id,order_type=order['type'],lots=order['lots'],price=order['open_price'],stop_loss=order['SL'],take_profit=order['TP'],magic=order['magic'],comment=order['comment'])
    ...
    def open_order(self, symbol='EURUSD',
                   ...
                   take_profit=0,
                    ...
                    comment='',
                    expiration=0):
        # move mouse cursor to center position
        pg.moveTo(960,540)
        pg.click()
        # pg.click(terminal.right,None)
        #press space button
        pg.press("space")
        # write the symbol
        pg.typewrite(symbol+",5")
        # press enter
        pg.press("enter")
        ...
        pg.typewrite(lots)
        pg.press("tab")
        pg.typewrite(str(stop_loss))
        pg.press("tab")
        pg.typewrite(str(take_profit))
        pg.press("tab")
        pg.press("tab")
        pg.typewrite(str(comment))
        ...
        pg.sleep(4)
        try:
            reqoute=pg.locateOnScreen("reqoute.png",confidence=0.9)
            pg.center(reqoute)
            pg.click(reqoute)
        ...
        while True:
            try:
                time=pg.locateOnScreen("time tab2.png",confidence=0.99)
                if(time is not None):
                    break
            except pg.ImageNotFoundException:
                try:
                    time2=pg.locateOnScreen("time tab.png",confidence=0.99)
                    pg.click(time2)
                except pg.ImageNotFoundException:
                    try:
                        time1=pg.locateOnScreen("time tab1.png",confidence=0.99)
                        pg.click(time1)
                    except pg.ImageNotFoundException:
                        print("")
        try:    
            ticket=pg.locateOnScreen("ticket.png",confidence=0.9,grayscale=True)
            pg.moveTo(ticket)
            # move down to the ticket
            pg.moveRel(0,ticket.height)
            pg.click()
            pg.screenshot(imageFilename=str(order_id)+".png",region=(int( ticket.left),int( ticket.top+(ticket.height+4)),int( ticket.width),int( ticket.height)-4))
        except pg.ImageNotFoundException:
    ...
    def close_order(self, ticket):
        ...
        try:    
            ticketTab=pg.locateOnScreen("ticket.png",confidence=0.9)
            pg.moveTo(ticketTab)
            # move down to the ticket
            pg.moveRel(0,ticketTab.height )
            pg.click()
        ...
        while True:
            try:
                order=pg.locateOnScreen(str(ticket)+".png",confidence=0.99)
                pg.center(order)
                # press fn + ctrl 
                # pg.press("menu")
                pg.rightClick(order)
                ...
                x,y=pg.position()
                try:
                    Balance=pg.locateOnScreen("balance.png",confidence=0.9)
                except pg.ImageNotFoundException:
                    print("Balance not found")
                if(Balance.top<=y):
                    os.remove(str(ticket)+".png")
                    break
                pg.click()
        pg.sleep(1)
        try :
            close=pg.locateOnScreen("closeOrder.png",confidence=0.9 )
            pg.center(close)
            pg.click(close)
